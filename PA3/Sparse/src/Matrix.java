...
public class Matrix {
	...
	Matrix add(Matrix M) {
		...
		Matrix temp = copy();
		Matrix temp2 = M.copy();//avoid case when M = Current
		for (int i = 1; i < size; i++) {
			if (temp2.row[i].length() > 0) { //find the numbers in M in order
				temp2.row[i].moveFront();
				while (temp2.row[i].index() >= 0) {
					double a = ((Entry) temp2.row[i].get()).val; //record the first number in M
					if (row[i].length() > 0) {
						row[i].moveFront();
						while (row[i].index() >= 0) {
							if (((Entry) row[i].get()).col == ((Entry) temp2.row[i]
									.get()).col) {
								a += ((Entry) row[i].get()).val; //if there is a number in current has same column
								break;							//add that value to a
							}
							row[i].moveNext();
						}
					}
					temp.changeEntry(i, ((Entry) temp2.row[i].get()).col, a);
	...
	Matrix sub(Matrix M) {// returns a new Matrix that is the difference of this
		...
		Matrix temp = copy();
		temp = temp.scalarMult(-1); // A-B = A + (-B)
		Matrix sub = M.add(temp);// Add the negative temp to M
		sub = sub.scalarMult(-1);
		return sub;
	}
